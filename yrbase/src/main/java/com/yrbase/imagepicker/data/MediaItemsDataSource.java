            ...
            public void run() {
                ...
                if (!context.isDestroyed() && !cursor.isClosed() && cursor.moveToFirst()) {
                    do {
                        ...
                        long aLong = getLong(cursor, MediaStore.Video.Media.SIZE);

                        long width = getLong(cursor, MediaStore.Video.Media.WIDTH);

                        long height = getLong(cursor, MediaStore.Video.Media.HEIGHT);

                        if (aLong>500&&width>200&&height>200) {
                            imageItems.add(item);
                        }
